<template>
  <UCard
    :key="item.title"
    variant="solid"
    class="flex flex-col items-center bg-neutral-50 w-md rounded-md pb-4 pt-0 px-0 border-3 border-[color:var(--ui-bg-contrast)] overflow-hidden"
    :data-aos="item.dataAos"
    :data-aos-duration="item.dataAosDuration"
    >
    <template #header>
      <div class="flex flex-col justify-center items-start gap-4 -mx-6 -mt-6">
      <LazyNuxtImg :src="item.src" class="w-full border-b-3 border-b-[color:var(--ui-bg-contrast)]" />
      <div class="flex items-center gap-3 px-4">
        <div
        class="w-12 h-12 rounded-full flex items-center justify-center bg-primary"
      >
      <UIcon
      :name="
